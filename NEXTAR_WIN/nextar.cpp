...
int DrawGLScene(float sceneTime)									
{
	...
	//bg::DrawPlasma(sceneTime*0.05f);
	//bg::DrawDistort(sceneTime*0.05f);
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
    ...
    //wc.hIconSm = LoadIcon (hInstance, MAKEINTRESOURCE(IDI_RESFUND));
				...
				int		nCmdShow)		// Window Show State
{
	...
	if (!CreateGLWindow("Fabio's OpenGL Framework",640,480,16,fullscreen))
	{
		...
		if (PeekMessage(&msg,NULL,0,0,PM_REMOVE))	
		{
			...
			if (active)
			{
				if (keys[VK_ESCAPE])				
				{
					...
					sceneTime += 0.01f;
					if(sceneTime > 10) sceneTime = 0;
					DrawGLScene(sceneTime);					
					SwapBuffers(hDC);				
				}
			}
			if (keys[VK_F1])						
