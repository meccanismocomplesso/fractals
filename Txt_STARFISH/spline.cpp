...
/* Modified March 2006 by The Blue Ant */
...
SPLINE* make_spline (unsigned int size)
{
  ...
  s->points = (POINT*)calloc (s->allocated_points, sizeof (POINT));
...
static void grow_spline_points (SPLINE *s)
{
  ...
  s->points =
    (POINT*)realloc (s->points, s->allocated_points * sizeof (POINT));

}

static void mid_point (double x0, double y0,
	   double x1, double y1,
	   double *mx, double *my)
{
  *mx = (x0 + x1) / 2.0;
...
static int can_approx_with_line (double x0, double y0,
		      double x2, double y2,
		      double x3, double y3)
{
	  ...
	  double x1, double y1)
{
  ...
  if (s->n_points == 0)
    {
      s->points [s->n_points].x = (long)x0;
      s->points [s->n_points].y = (long)y0;
      s->n_points += 1;
    }
  s->points [s->n_points].x = (long)x1;
  s->points [s->n_points].y = (long)y1;
  s->n_points += 1;
}
...
static void calc_section (SPLINE *s,
...
void just_fill_spline (SPLINE *s)
{
  ...
  for (i = 0; i < s->n_controls; i++)
    {
      s->points [i].x = (long)s->control_x [i];
      s->points [i].y = (long)s->control_y [i];
    }
  s->points [s->n_controls].x = (long)s->control_x [0];
  s->points [s->n_controls].y = (long)s->control_y [0];
...
void spline_bounding_box (SPLINE *s, RECT *rectangle_out)
{
  ...
  if (s->n_points == 0)
    {
      rectangle_out->left = 0;
      rectangle_out->top = 0;
      rectangle_out->right = 0;
      rectangle_out->bottom = 0;
    }
  ...
  rectangle_out->left = min_x;
  rectangle_out->top = min_y;
  rectangle_out->right = max_x;
  rectangle_out->bottom = max_y;
}
